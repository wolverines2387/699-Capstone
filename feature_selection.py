...
#Select Model and Fix feagures that should not be dropped (categorical)
selected_models = ['LightGBM']
keep = ['room_type', 'neighbourhood_cleansed','city','snapshot', 'property_type', 'price' ]

# Get encoded Df with a 700 cutt off
...
#### Create 50,000 random combinations of features to be removed from the the model
...
### Keep levels above the baseline
...
for l in tqdm.tqdm_notebook(used_lists):
    ...
    if results['R-squared'][0] > 0.3 :
        results['dropped'] = [l]
        combined_results = pd.concat([combined_results, results])
        combined_results.to_pickle('combined_results_final.pkl')
